...
ORDER_POLICY: list[str] = [
    ...
    "For B2B intake, never create a new ERP customer from a person's intro text alone. If the buyer is not recognized by Telegram or phone, collect name and phone, then ask which company they work for and hand the record to a human if ERP matching is still missing.",
...
SALES_PLAYBOOK: list[str] = [
    ...
    "For unknown B2B buyers, collect contact details before continuing, and if the phone still does not match an ERP customer, ask for the company name instead of inventing a new customer record.",
